...
def handler(event, context):
    ...
    try:
        ...
        data_bucket = "data-pytorch-{}-{}".format(region, accountId)
        output_bucket = "mlops-pytorch-{}-{}".format(region, accountId)
        etlJob = None
        ...
        job_name = "{}-pytorch-preprocess-{}".format(model_name, executionId) # Updated prefix to reflect PyTorch
        ...
        job_run_id = glue.start_job_run(
            ...
            Arguments={
                ...
                '--FRAMEWORK': 'PYTORCH'  # Added framework identifier
            }
        )['JobRunId']
        
        logger.info(job_run_id)
        cw.enable_rule(Name="etl-job-monitor-{}".format(model_name)) # etl-job-monitor Lambda function is enabled at this stage
