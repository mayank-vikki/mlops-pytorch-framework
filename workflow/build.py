...
def get_workflow_role():
    ...
    """
    try:
        name = f"{os.environ['WORKFLOW_NAME']}-WorkflowExecRole"
...
def get_lambda_arn(name):
    ...
    """
    try:
        full_name = f"{os.environ['WORKFLOW_NAME']}-{name}"
...
if __name__ == "__main__":
    ...
    # Python's argparse module automatically converts the hyphenated command-line argument --pipeline-bucket 
    # to an underscore-separated attribute name pipeline_bucket when you access it through the args object.
    ...
    parser.add_argument("--target-column", type=str, default=os.environ["TARGET_COLUMN"])
    ...
    evaluate_endpoint_step = LambdaStep(
        "Evaluate SageMaker Hosted PyTorch Model",
        parameters={
