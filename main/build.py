...
def get_approved_package(package_group_name):
    ...
    try:
        ...
        if len(approved_packages) == 0:
            warning_message = (
                f"⚠️ No approved ModelPackage found for ModelPackageGroup: {package_group_name}"
            )
...
def extend_prd_params(args, stage_config):
    ...
    approved_package = get_approved_package(args.model_package_group_name)
    if not approved_package:
        logger.warning(f"No approved model found for group {args.model_package_group_name}. Skipping PRD stage.")
        return None  # or return a stub config if needed

    params = {
        "ModelName": args.model_name,
        "TrainJobId": get_job_id(deployment_stage, args.pipeline_name),
        "ModelPackageName": approved_package
    }
    return {
        "Parameters": {**stage_config["Parameters"], **params}
    }
    ...
    if deployment_stage == 'Dev':
        ...
        with open(args.import_config, "r") as f:
            try:
                ...
                logger.warning(f"⚠️ Skipping PRD config generation: {e}")
                sys.exit(0)
    else:
